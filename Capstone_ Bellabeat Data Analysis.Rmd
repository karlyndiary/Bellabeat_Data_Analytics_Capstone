...
str(sleep_day)
```
### 3.8 Dropping total_minutes_asleep in sleep_day dataset
```
sleep_day = subset(sleep_day, select = -c(total_minutes_asleep))
```

### 3.9 Formatting String datatype to Boolean datatype
```{r}
weight_log_info$is_manual_report <- as.logical(weight_log_info$is_manual_report)

str(weight_log_info)
```
### 3.10 Creating a new column for weekdays
```{r}
...
### 3.11 Merging daily_activity and sleep_day dataset
```{r}
